...
const Home = () => {
  ...
  useEffect(() => {
    const fetchWeatherData = async () => {
      try {
        ...
        // console.log('Information data fetched:', response.data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5000/information", newWeather);
  ...
  const handleDelete = async (_id) => {
    try{
      await axios.delete(`http://localhost:5000/information/${_id}`);
      setWeathers(weathers.filter(weather => weather._id !== _id));
              ...
              <div className="card mb-4 ">
                <div className="card-body">
                  <h4 className="card-title">
                    {weather.country}
                      <button className="btn btn-danger float-end btn-sm"><FaTrash onClick={()=> handleDelete(weather._id)}/></button>
                  </h4>
